    ...
    <NavLink to={to} end={end} className={({ isActive }) => `nav-item ${isActive ? "active" : ""}`}>
      {({ isActive }) => (
        <>
          ...
          {badge && unread > 0 && (
            <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full text-white animate-glow-pulse"
              style={{ background: "var(--danger)", boxShadow: "0 0 8px rgba(248,113,113,0.4)" }}>
              {unread > 9 ? "9+" : unread}
            </span>
          )}
        </>
  ...
  return (
    <aside className="w-60 min-h-screen flex flex-col flex-shrink-0 relative"
      ...
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: "linear-gradient(180deg, rgba(129,140,248,0.02) 0%, transparent 40%)" }} />

      {/* Logo */}
      <div className="px-4 py-5 relative z-10" style={{ borderBottom: "1px solid var(--border)" }}>
      ...
      <div className="px-3 py-4 relative z-10" style={{ borderTop: "1px solid var(--border)" }}>
        {user && (
          <div className="flex items-center gap-2.5 mb-3 px-1">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center text-[11px] font-bold flex-shrink-0"
              style={{ background: "linear-gradient(135deg, var(--accent-bg), rgba(168,85,247,0.12))", color: "var(--accent-light)", border: "1px solid var(--accent-border)" }}>
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl"
          style={{ background: "linear-gradient(135deg, var(--accent-bg), rgba(168,85,247,0.05))", border: "1px solid var(--accent-border)" }}>
          <Sparkles style={{ width: 10, height: 10, color: "var(--accent)" }} />
          <span className="text-[11px] font-medium" style={{ color: "var(--text-muted)" }}>v1.0</span>
          <span className="ml-auto text-[9px] font-bold px-2 py-0.5 rounded-full"
            style={{ background: "var(--success-bg)", color: "var(--success)", border: "1px solid var(--success-border)" }}>LIVE</span>
        </div>
      </div>
    </aside>
  );
